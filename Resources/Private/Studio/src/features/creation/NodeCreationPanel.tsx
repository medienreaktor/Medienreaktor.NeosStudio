...
import { isOfType, useNodeTypeGroups, useNodeTypes, type NodeTypeDto } from '@/api/nodeTypes'
...
export function NodeCreationPanel() {
  ...
  const groups = useMemo(() => {
    ...
    const byGroup = new Map<string, { nodeType: NodeTypeDto; label: string }[]>()
    for (const nodeType of nodeTypes.values()) {
      ...
      const label = humanizeLabel(nodeType.label, nodeType.name.split(':').pop() ?? nodeType.name)
    ...
    const configuredOrder = sortByPosition(
      Object.entries(groupConfigs).map(([key, config]) => ({ key, position: config?.position })),
    )
    const unconfigured = [...byGroup.keys()].filter((name) => !configuredOrder.includes(name)).sort()
    ...
    return [...configuredOrder, ...unconfigured]
      ...
      .map((name) => ({
        ...
        nodeTypes: sortByPosition(
          byGroup.get(name)!.map(({ nodeType }) => ({ key: nodeType.name, position: nodeType.position })),
        ).map((typeName): CreatableNodeType => {
          const entry = byGroup.get(name)!.find((candidate) => candidate.nodeType.name === typeName)!
          return { name: typeName, label: entry.label, icon: entry.nodeType.icon }
        }),
      }))
  }, [nodeTypes, groupConfigs])

  if (groups === null) {
    return <div className="p-4 text-xs text-muted-foreground">Loading node types…</div>
  }
    ...
    .map((group) => ({
      ...
      nodeTypes: query
        ? group.nodeTypes.filter(
            (nodeType) =>
              nodeType.label.toLowerCase().includes(query) || nodeType.name.toLowerCase().includes(query),
          )
        : group.nodeTypes,
    }))
    ...
    <div className="@container flex flex-col gap-2 p-2">
      ...
      {visibleGroups.length === 0 && (
        <p className="px-1 text-xs text-muted-foreground">
          {groups.length === 0 ? 'No creatable content node types.' : 'No node types match the filter.'}
        </p>
      )}
      {visibleGroups.map((group) => {
        // While filtering every group with matches stays open.
        const isCollapsed = query === '' && (toggled[group.name] ?? group.collapsed)
        return (
          <section key={group.name}>
            <button
              ...
              onClick={() => setToggled((previous) => ({ ...previous, [group.name]: !isCollapsed }))}
            >
